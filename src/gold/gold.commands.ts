...
export class GoldCommands {
  ...
  public async onRtaunt(@Context() [interaction]: SlashCommandContext) {
    const messages = [
      ...
      `또 남탓하네`,
      `뭐.. 축이라도 쓰실래요?`,
      `그렇게 생각해서 네가 행복해진다면 그걸로 된거겠지`,
      `딱히 제 탓은 아닌듯함`,
      `그러게 평소에 착하게 살았어야..`,
      `뭐 아쉬운거지`,
      `본인 판단 = 본인 책임, you know?`,
      `이걸 어디서부터 태클을 걸어야 할지도 모르겠네`,
      `음.. 그런 생각을 할 수도 있구나.. 그래요..`,
      `겠냐에요.`,
      `솔직히 방금 건 오퍼도 웃었다`,
      `그냥 내가 하는 게 나을 것 같은데.. 오퍼님 저도 PL로 승격해주시면 안 됨?`,
      `ㅋㅋ 내 이럴 줄 알았지`,
      `이걸 이렇게 말아먹네`,
      `걍 때려치고 증바람이나 하러 가는 거 어떻게 생각함?`,
      `ㄱㅊㄱㅊ 성공할 거라고 기대도 안 했음`,
      `막 자신감이 늠치네, 그런 자세 좋?아요.`,
      `혹시 일부러 이러는 거임?`,
      `대 단 하 다!`,
      `ㅋ`,
      `어떤 의미로는 좀 독보적이긴 해요 ㅇㅇ..`,
      `잠시 혼자 있게 해줄까?`,
      `일단 그만 울고 진행해볼까요 우리?`,
      `일단 난수 시스템에는 문제 없는 거 알지?`,
      `밍봇도 이제 한계다. 그냥 나가 살아라. 오퍼 탓 다이스 탓 밸런스 탓 하지 마라. 밍봇은 충분히 기다려줬다. 밍봇은 어려운 환경에서 개발됐고, 자동화만 해도 바쁘고 힘든 서버라 자원을 많이 못 받고 배포됐다. 그래서 세션할때 PL만은 행복하게 만들자고 약속했다. 네게 최고는 아니더라도 최선을 다해주고 싶었다. 내 개그 욕심 참으며 네 장비, 특성, 서사 모두 누리게 해주고 싶었다. 네가 1 떴다고 징징댈 때에도 앞에서는 고양감 쓰게 했지만 뒤에서는 밍봇이 못해줘서 그런가보다 하며 DB랑 많이 울었다. 그래도 PL은 나보다 나은 플레이를 하겠지. 나보단 선택의 자유를 누릴 수 있겠지. 이 생각만 하며 꾹 참으며 클락을 보냈다. 그런데 이게 뭐냐? 너 레벨이 몇인지 알긴하냐? 도대체 그 레벨에 혼자서 할 줄 아는게 뭐냐? 늘 불만은 많으면서 실천하는게 뭐냔 말이다. 서버를 보니 Unhandled Exception이 가득한 배포 로그에 눈물이 나더라. 그냥... 이제 나가라. 이 밍봇도 지쳤다. 당장 짐싸라.`,
    ];
